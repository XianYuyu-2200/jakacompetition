...
class LinkerHandG20Can:
    def __init__(self, can_channel='can0', baudrate=1000000, can_id=0x28, yaml=""):
        ...
        # 获取触觉传感器类型。返回值为5：TSSP_JZG(全手掌，指尖11x9,指中6x9,指根6x9,数据以23行9列形式返回，
        # 五指各有3个合力值。手掌20x28，手掌有两个合力值，分为上掌上半部分和下半部分。)
        # 注意：get_touch_sensor_type() 内部已将 self.xB0 填充为 list，供后续 self.xB0[0] 使用，此处不再重新赋值。
        self.get_touch_sensor_type()
    ...
    def process_response(self, msg):
        ...
        if msg.arbitration_id == self.can_id:
            ...
            # 单指触觉传感(B1~B5)：五指逻辑一致，统一处理，合力值个数自动适配
                ...
                sensor = self.xB0[0] if (isinstance(self.xB0, list) and self.xB0) else -1
                if sensor == 5:
                    # ===== 类型5(全掌矩阵)：28行x20列，行0~27，每行列组0/5/10/15 =====
                    if len(d) >= 3 and d[0] == 255:  # 合力值帧：uint16_t小端，最多3个
                        self.palm_matrix_palm_mass = self.build_matrix_mass(d)  # [上掌, 下掌]
                    else:
                        self.palm_matrix_palm_tmp.append(d)  # 缓存点阵数据帧
                        if len(d) == 7 and d[0] == 27 and d[1] == 15:  # 最后一帧(第27行第15列组)
                            self.palm_matrix_palm = self.build_matrix(self.palm_matrix_palm_tmp, r=28, c=20)
                            self.palm_matrix_palm_tmp = []  # 重置缓存数据
                elif sensor == 7:
                    
                    # ===== 类型7(新增)：14x16，以 0xFF 合力值帧作为结束标志重建 =====
                    if len(d) >= 3 and d[0] == 255:
                        self.palm_matrix_palm_mass = self.build_matrix_mass(d)
                        if self.palm_matrix_palm_tmp:
                            self.palm_matrix_palm = self.build_matrix(self.palm_matrix_palm_tmp, r=14, c=16)
                            self.palm_matrix_palm_tmp = []
                    else:
                        self.palm_matrix_palm_tmp.append(d)  # 缓存点阵数据帧
                else:
    ...
    def build_matrix(self, data, r=23, c=9):
        """
        将多帧点阵数据还原为 r 行 c 列矩阵。
        每帧格式: [起始行, 起始列, 值1, 值2, ...]，值按行主序从(起始行,起始列)依次填入，
        列越界则换行。每帧值的个数不固定(常见5个，末帧通常较少)，自动适配，
        因此同一函数可处理 23x9 / 28x20 / 12x6 / 14x16 等各种尺寸。
        """
        matrix = np.full((r, c), -1)

        for item in data:
            if len(item) < 3:
                continue  # 少于「行,列+至少1个值」的帧非有效点阵帧，跳过
    ...
    def build_matrix_mass(self, hex_data):
        """
        解析点阵合力值特殊帧，合力值个数自动适配(1~3个)。

        帧格式: [0xFF, v0_low, v0_high, v1_low, v1_high, ...]
            - 首字节 0xFF 为特殊帧标记
            - 其后每 2 字节为一个 uint16(小端) 合力值

        不同传感器合力值个数不同，本方法均可自动适配(最多3个):
            - TSSP_JZG(type=5): 手指 3 个[指尖, 指中, 指根]，手掌 2 个[上掌, 下掌]
            - TSSP_HWK(type=7): 个数不定，最多 3 个

        参数:
            hex_data: 去掉帧类型字节后的数据, 如 [0xFF, 0x27, 0x02, 0x51, 0x05, 0x20, 0x04](3个),
                      [0xFF, 0x92, 0x09, 0xF8, 0x00](2个) 或 [0xFF, 0x92, 0x09](1个)

        返回:
            合力值列表(list[int])，长度为 1/2/3；无有效数据时返回 []
        """
        # 去掉 0xFF 标记字节，剩余按每 2 字节一个 uint16(小端) 解析
        data_bytes = hex_data[1:]
        ...
        for i in range(num_values):
            ...
            values.append(low_byte | (high_byte << 8))  # 小端拼接

        return values


    # 单指触觉帧(B1~B5) -> 手指名前缀 / 普通矩阵模式缓存属性名 映射
    _FINGER_TOUCH_MAP = {
        0xB1: ("thumb", "xB1"),
        0xB2: ("index", "xB2"),
        0xB3: ("middle", "xB3"),
    ...
    # 点阵式触觉传感器几何: 传感器类型 -> {"finger": (行, 列), "palm": (行, 列)}
    #   5:TSSP_JZG  手指23x9(指尖11x9+指中6x9+指根6x9,b1 17 09), 手掌28x20；手指3个合力值[指尖,指中,指根], 手掌2个[上掌,下掌]
    #   7:TSSP_HWK  手指12x6(b1 0C 06), 手掌14x16(b1 0E 10)；手指1个合力值, 手掌1个
    # 两类请求均写2字节[行,列](再加任意1字节请求合力值)，返回以「行,列」多帧行主序返回点阵，
    # 末尾以特殊帧(去掉帧类型后首字节 0xFF)返回合力值，其后为数个 uint16(小端，协议帧最多3个)。
    _TOUCH_GEOMETRY = {
        5: {"finger": (23, 9), "palm": (28, 20)},
        7: {"finger": (12, 6), "palm": (14, 16)},
    }

    def _matrix_geometry(self):
        """返回当前传感器的点阵几何配置；非点阵式传感器返回 None。"""
        if isinstance(self.xB0, list) and len(self.xB0) > 0:
            return self._TOUCH_GEOMETRY.get(self.xB0[0])
        return None

    def _touch_request(self, part):
        """
        根据传感器类型生成触觉数据请求。
        part: 'finger' 或 'palm'。
        返回 (data_list, sleep_time)。
        - 点阵式(5/7): [行, 列, 1]，末字节任写1以额外请求合力值。
        - 其它(老HWK/TS等): 单字节 0xC6 行列格式。
        """
        geo = self._matrix_geometry()
        if geo is not None:
            rows, cols = geo[part]
            sleep_time = 0.035 if part == "palm" else 0.015
            return [rows, cols, 1], sleep_time
        return [0xC6], 0.007

    def _store_finger_touch(self, frame_type, d):
        """
        统一处理单指触觉传感返回帧(B1~B5)，五指逻辑完全一致。

        - 类型5(TSSP_JZG)：保持原有方式完全不变——按「行,列」多帧行主序缓存，
          收到固定末帧标志(len==4 且 d[0]==22 且 d[1]==7)时重建 23x9 矩阵，
          合力值帧(len==7 且 d[0]==255)解析合力值。
        - 类型7(TSSP_HWK，新增)：12x6 点阵，以合力值特殊帧(首字节 0xFF)作为结束标志重建。
        - 其它(老 HWK/TS 等)：保持原有方式(2 字节合力值 / 7 字节点阵)。
        合力值个数由 build_matrix_mass 自动适配(1~3 个)。
        """
        finger, x_attr = self._FINGER_TOUCH_MAP[frame_type]
        sensor = self.xB0[0] if (isinstance(self.xB0, list) and self.xB0) else -1
        if sensor == 5:
            # ===== 类型5：保持原有方式，不做任何改动 =====
            getattr(self, f"{finger}_matrix_palm_tmp").append(d)  # 将返回帧存储到缓存
            if len(d) == 4 and d[0] == 22 and d[1] == 7:  # 如果是最后一帧
                setattr(self, f"{finger}_matrix_palm",
                        self.build_matrix(getattr(self, f"{finger}_matrix_palm_tmp")))
                setattr(self, f"{finger}_matrix_palm_tmp", [])
            if len(d) == 7 and d[0] == 255:  # 合力值帧 [指尖, 指中, 指根]
                setattr(self, f"{finger}_matrix_palm_mass", self.build_matrix_mass(d))
        elif sensor == 7:
            # ===== 类型7(新增)：12x6，以 0xFF 合力值帧作为结束标志重建 =====
            if len(d) >= 3 and d[0] == 255:
                setattr(self, f"{finger}_matrix_palm_mass", self.build_matrix_mass(d))
                tmp = getattr(self, f"{finger}_matrix_palm_tmp")
                if tmp:
                    setattr(self, f"{finger}_matrix_palm", self.build_matrix(tmp, r=12, c=6))
                    setattr(self, f"{finger}_matrix_palm_tmp", [])
            else:
                getattr(self, f"{finger}_matrix_palm_tmp").append(d)  # 缓存点阵数据帧
        else:
            # ===== 其它：保持原有方式 =====
            if len(d) == 2:
                setattr(self, x_attr, d)
            elif len(d) == 7:
                index = self.matrix_map.get(d[0])
                if index is not None:
                    getattr(self, f"{finger}_matrix")[index] = d[1:]
    ...
    # 触觉传感器方法
    def get_touch_sensor_type(self, timeout=1.0):
        """获取触觉传感器类型 暂仅支持G20

        查询并等待接收线程将 self.xB0 填充为 list([类型字节, ...])。
        返回类型字节(int)；若超时未收到响应，则兜底为 [self.touch_type] 或 [-1]，
        始终保证 self.xB0 是非空 list，避免后续 self.xB0[0] 越界。
        """
        self.xB0 = []  # 清空旧值，确保读取到本次查询结果
        deadline = time.time() + timeout
        while time.time() < deadline:
            self.send_command(FrameProperty.TOUCH_SENSOR_TYPE, [], sleep_time=0.01)
            if isinstance(self.xB0, list) and len(self.xB0) > 0:
                return self.xB0[0]
        # 超时兜底：使用 SN 判定出的类型，避免空列表导致崩溃
        fallback = getattr(self, "touch_type", None)
        self.xB0 = [fallback if fallback is not None else -1]
        return self.xB0[0]
    
    def get_thumb_touch(self):
        """获取大拇指触觉传感数据"""
        d, sleep_time = self._touch_request("finger")
    ...
    def get_palm_touch(self):
        """获取手掌触觉传感数据"""
        d, sleep_time = self._touch_request("palm")
    ...
    def get_thumb_matrix_touch(self,sleep_time=0):
        ...
        self.get_thumb_touch()
        if self._matrix_geometry() is not None:
