...
TOKEN="${1:-}"
DURATION="${2:-20}"
if [[ "$TOKEN" != "START_SIX_JOINTS" ]]; then
  echo "Usage: $0 START_SIX_JOINTS [duration_sec]" >&2
  exit 64
fi
...
echo "Six-joint follow test: $OPERATOR_IP -> $TRACKING_IP"
echo "J1-J6 relative motion is 1:1"
echo "No automatic robot power-on or robot enable"

"$ROOT/six_joint_follow_test" \
  "$TRACKING_IP" "$IPC_SOCKET" START_SIX_JOINTS "$DURATION" &
FOLLOWER_PID=$!

for _ in $(seq 1 200); do
  [[ -S "$IPC_SOCKET" ]] && break
  if ! kill -0 "$FOLLOWER_PID" 2>/dev/null; then
    echo "six-joint follower exited before creating IPC socket" >&2
    exit 1
  fi
  sleep 0.01
done
...
echo "six-joint follower exit code: $FOLLOWER_RC"
