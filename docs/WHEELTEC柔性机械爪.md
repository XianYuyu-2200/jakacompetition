...
> 这张表是按"工具轴竖直向下、末端正好落在该点"算的严格判据。工位 4/6 的
> **预抓取**一行虽然是 ✘, 但它不会直接让抓取失败(见下面的实测: 参考实现有
> 位姿兜底); 真正致命的是**夹起工件后必须竖直抬到的那 232.6mm**。
...
>
> 一整轮跑下来: 6 个工位**都抓到了**工件(夹爪张合、附着都正常)。
> 半径 ≤355mm 的工位 1/2/3/5 全流程正常 —— 抓取点、预抓取点、转场高度都落在
> 上限以内。工位 4/6 则卡在**竖直抬到转场高度**那一步:
> `lift(±0.150, 0.355, 0.233) -> FAIL(error_code=99999)`, RRTConnect 直接报
> `Unable to sample any valid states for goal tree`, 重试 3 次后放弃
> (预抓取/下降那两步之所以还能"凑合"完成, 是因为 IK 无解时参考实现会退化成
> "只给位姿、让规划器自己找解"的兜底模式 —— 代价是下降的笛卡尔直线只走完 33%)。
> 最后成绩是"1 件顺序正确 + 3 件因 6 号没进盒而判错序", 145s 超时。
> 所以卡住的不是夹爪, 是"在 r=385mm 上抬不到 233mm 高"。

> 也就是说:用这只夹爪,赛道一的 6 个工位**都夹得住**(指尖够深、工件坐得进去),
> 但半径最大的两个工位(4、6)夹起来之后**抬不到转场高度**, 工件就进不了料盒。
> **这不是仿真参数没调好, 是 580mm 臂展 + 164.6mm 刀尖的几何结果** ——
> 真机同样抬不起来。
...
| **先内收再抬高**(只改运动策略) | 竖直抬升之所以失败,是因为那条直线整段都在 r=385mm 上。先沿水平方向把工件朝基座收 ≈40mm,再抬到 232.6mm —— r 掉到 355mm 以下就够得到了 | 工件要贴着工位板/邻件平移一段,更容易蹭到旁边的工件;参考实现里要单独加这条"L 形转场"路径 |
