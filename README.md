...
- **末端多长,直接决定够得到哪里**:规则书要的是行程 0–30mm 的小平行夹爪,所以
  ...
  高度抬 ~74mm,`z_max(r) = 27.7 + √(420.5² − r²)` mm —— 工位 4/6(r=385mm)的
  预抓取悬停(208.6mm)和"原地竖直抬到转场高度"(232.6mm)都超出该半径上限
  195.9mm。参考实现按工业做法绕开: 悬停压到上限、转场走**L 形**(先抬→沿半径
  内收→再抬),实测柔性爪也能 6/6 满分。`arena_check` 会逐点报出这些高度。
  用法与可选方案见 `docs/WHEELTEC柔性机械爪.md` 第 4 节。
